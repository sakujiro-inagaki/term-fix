# term-key Specification

## Purpose

Define the logical `Key` union (printable characters, special keys, navigation, function keys, Ctrl/Alt modifiers) and a pure byte-buffer state machine (`parse : Array U8 -> KeyParseResult`) that converts raw stdin bytes into recognised keys, handles CSI / SS3 escape sequences, supports UTF-8 multi-byte input, and reports `incomplete` / `invalid` so that the caller can drive resync and bare-ESC timeout decisions.
