# term-mode Specification

## Purpose

Provide safe entry to and exit from POSIX `termios` modes (raw, raw-without-signals, no-echo) on stdin, expose an opaque `TermMode` snapshot value for explicit save/restore, and provide an exception-safe `with_raw` scope that guarantees the previous termios is restored even when the inner action fails — preventing the common "broken terminal after Ctrl+C" failure mode.
