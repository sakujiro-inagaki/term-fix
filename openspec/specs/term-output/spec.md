...
Provide simple, always-flushed write helpers for stdout — a single-string `write` and a single-syscall batched `write_batch` — so that TUI applications can emit ANSI sequences and frame contents without tearing and without forgetting to flush.
